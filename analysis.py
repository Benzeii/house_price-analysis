...
print("Running code updated at 04:55 PM BST, July 14, 2025")
...
df_test['title'] = X_test.index  # Using index as a proxy for house ID
df_test['actual_revenue'] = y_test_original
df_test['predicted_revenue'] = y_pred_original
df_test.to_csv('predictions_houses.csv', index=False)
...
# Load cleaned data and insert into table (simplified for now)
df_clean = df[['LotArea', 'YearBuilt', 'OverallQual', 'TotalBsmtSF', 'GrLivArea', 'GarageArea', 'SalePrice']].copy()
df_clean.to_sql('houses', conn, if_exists='replace', index=True, index_label='id')

# Update with predictions
for idx, row in df_test.iterrows():
    cursor.execute('UPDATE houses SET predicted_revenue = ? WHERE id = ?', (row['predicted_revenue'], row['title']))

# Verify
cursor.execute('SELECT id, revenue, predicted_revenue FROM houses WHERE predicted_revenue IS NOT NULL LIMIT 5')
